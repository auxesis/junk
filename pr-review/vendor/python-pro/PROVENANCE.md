# python-pro — vendored skill

Verbatim copy of the upstream `python-pro` skill, kept here so the MIT notice
...
Files: `SKILL.md`, `references/async-patterns.md`, `references/packaging.md`, `references/standard-library.md`, `references/testing.md`, `references/type-system.md`.
...
`src/pr_review/review_types/python.py` holds a review rubric **distilled** from these
...
was derived from, and they back the `--review-type python` prompt.

## Re-syncing

```bash
mise run sync-vendor                    # every skill, from main
mise run sync-vendor SKILL=python-pro   # just this one
mise run sync-vendor REF=v2             # or a tag / sha
```

Then read the diff, decide whether the rubric in `python.py` needs to change,
