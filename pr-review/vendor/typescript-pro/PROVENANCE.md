# typescript-pro — vendored skill

Verbatim copy of the upstream `typescript-pro` skill, kept here so the MIT notice
...
Files: `SKILL.md`, `references/advanced-types.md`, `references/configuration.md`, `references/patterns.md`, `references/type-guards.md`, `references/utility-types.md`.
...
`src/pr_review/review_types/typescript.py` holds a review rubric **distilled** from these
...
was derived from, and they back the `--review-type typescript` prompt.

## Re-syncing

```bash
mise run sync-vendor                    # every skill, from main
mise run sync-vendor SKILL=typescript-pro   # just this one
mise run sync-vendor REF=v2             # or a tag / sha
```

Then read the diff, decide whether the rubric in `typescript.py` needs to change,
