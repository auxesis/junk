# golang-pro — vendored skill

Verbatim copy of the upstream `golang-pro` skill, kept here so the MIT notice
...
`src/pr_review/review_types/golang.py` holds a review rubric **distilled** from
these files — the skill teaches an agent to *write* Go, and a review type needs
...
```bash
mise run sync-vendor          # fetches main
mise run sync-vendor REF=v2   # or a tag/sha
```

Then read the diff, decide whether the rubric in `golang.py` needs to change, and
update the commit, version, and date above in the same commit.
